...
public class GdtApi {
    private static final String TAG = GdtApi.class.getSimpleName();
...
//                event.put("success", success ? 1 : 0);
            ...
            event.put("trackKey", GdtStatistics.trackKey);
            event.put("trackId", GdtStatistics.trackId);
...
//        report(openid, "pause", event, null);
    ...
    public void reportInit(String appId, String appName)
    {
        ...
        try
        {
            event.put("trackKey", appName);
            event.put("trackId", appId);
        }
        catch (JSONException e)
        {
            e.printStackTrace();
...
//        WebApi.startThreadRequest(REPORT, new ApiRequestListener()
//        {
...
//            {
//                Log.i(JrttApi.TAG, "report onError " + i);
//            }
//        }, params, AppConfig.appName);

        OkHttpManager.getInstance().postRequest(REPORT, params, new ResponseCallback<String>() {
            public void onSuccess(String o) {
                Log.i(GdtApi.TAG, "report onSuccess");
            }

            public void onFailure(OkHttpException e) {
                Log.i(GdtApi.TAG, "report onError " + e.getEmsg());
            }
        });
    }
}
